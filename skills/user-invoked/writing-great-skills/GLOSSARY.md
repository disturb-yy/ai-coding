...
A compact boundary map for a skill. It defines the target region the skill should pull the agent toward, the deviation region it should keep out of, the priority dimensions that decide tradeoffs, the entry and exit conditions, the pre-output check, and the sedimentation rule for what should be preserved or pruned after the run. It is useful when a skill's boundary is fuzzy, multi-branch, or likely to drift. It is a **no-op** when it only restates what the **description**, **leading word**, and **completion criterion** already make checkable.

Use Markdown prose plus a YAML block when the boundary needs to be scanned or edited as a stable object. Keep it co-located with the behaviour it bounds; disclose it only if some **branches** need it and others do not.

_Avoid_: scope, rubric, framework, checklist
