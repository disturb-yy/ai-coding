...
<!-- mirror-sync: 2026-07-09 hard-fail-skill-routing-rules -->

## 同步增量：skill 路由边界

- 当用户目标、验收标准、约束、业务规则、角色或交互不清，或用户要求“先把需求弄清楚”时，路由到 `grilling`，一次只问一个问题；如果仓库能回答，则改为探索仓库。
- `diagnosing-problem` 用于现象、原因、定位方法或问题框架不清的场景；不要用于功能需求澄清。
- 当产品或功能需求的规则、角色、交互或验收标准不清时，路由到 `grilling`，不要路由到 `diagnosing-problem`。
- 当 review/attack 请求建立在显式未验证的承重假设上时，先路由到 epistemic control，不要直接进入 adversarial review。
