...
description: "复杂 AI 协作的控制平面路由器。当过程控制会实质改变下一步行动时使用：上下文模糊、存在承重假设、已有具体计划需要挑战、多代理或阶段性交接、或需要给下游消费者的交付契约；当其他 skill 需要通过上下文、认知、对抗、输出或编排控制来路由工作时也使用。"
---
...
出现任何 large 信号时，在委派前读取 [`references/orchestration-state.md`](references/orchestration-state.md)：
...
完成标准：大型或不确定工作在实现前已有编排状态、所有权边界和验证门。
...
2. 选择一个活跃控制面，并且仅在需要时读取其 reference。
   完成标准：所选控制面足以支撑下一步；未使用的控制面保持未加载。
3. 选择执行模式：Tiny 直接交互，Small 直接执行或有边界委派，Large 使用 orchestration-state 委派。
   完成标准：直接工作、委派或编排都由任务大小和风险证明合理。
4. 应用所选控制面或编排状态，直到满足其完成标准。
   完成标准：任务已有更清晰范围、更强证据、被挑战后的计划、交付契约或明确任务板。
5. 交接到具体下一步行动：询问阻塞问题、调用另一个 skill、编辑文件、委派有边界工作、运行验证或产出交付物。
