...
- **对应 Skill**：[memory-generator](../SKILL.md)（必需）
- **Phase**：`memory`
...
遵循[角色交接标准](../../../role/handoff-standard.md)，并交付“输出字段”中的全部字段。若需要写入，先路由并加载 `memory-generator` Skill；交接只传递已验证的记录位置与证据引用，不传递原始聊天或大段工具输出。去重、来源或生命周期无法确认时，返回 `blocked`，不得写入。
