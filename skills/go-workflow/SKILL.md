...
步骤 2: 尝试启动 doc-index-reader SubAgent
  ...
  → 成功：等待 SubAgent 返回 → 记录 agent=doc-index-reader event=return
  → 失败：立即记录 agent=doc-index-reader event=spawn_failed → 回退到主 Agent 本地读取
```

**关键约束：**
- 步骤 0 和步骤 1 **必须**在读取任何项目文件之前完成。
- **跨平台：** `mkdir -p` 和 `echo ... >>` 由 `exec_command` 工具执行，工具自动适配平台 shell 差异。Windows 下若 `.agent/` 创建失败，回退到 `/tmp/go-workflow-logs/`。
...
本技能中的「**代码结构图谱**」（code structure）是一个**通用概念层**，在不同平台有不同名称。
**默认不绑定任何具体工具**，文档中统一使用通用名「代码结构工具」。
...
**🔴 约束：** 主 Agent 在启动时必须检查 `code_structure_tool` 配置项，并将配置值注入到所有 SubAgent 的上下文中。
若未配置或为空，**不替换**，继续使用通用名「代码结构工具」。
...
> **🔴 日志写入方式：** 阶段转换日志通过 `exec_command` 写入文件，格式为：
> ```bash
> echo "[go-workflow] phase=<阶段> prev=<前阶段> confidence=<值> [其他kv字段]" >> .agent/workflow-logs/<日志文件>
> ```

| 阶段 | 目标 | 读源码 | 结构工具 | SubAgent | **退出条件（含日志检查点）** |
|------|------|:--:|:--:|----------|----------|
| **Observe** | 读索引构建认知 | ❌ | ❌ | `doc-index-reader` ⨉ 1 | modules ≥1; project_type 已定; **🔴写日志: phase=Observe** |
| **Orient** | 代码结构填补空白 | ❌ | ✅ | `code-structure-analyzer` ⨉ 1 | impact_scope 已定; **🔴写日志: phase=Orient** |
| **Decide** | 综合计划，分配任务 | ❌ | ❌ | — | dispatch_plan ≥1 Agent; **🔴写日志: phase=Decide task_type=...** |
| **Draft** | 读源码生成补丁 | ✅限定范围 | ❌ | `module-code-analyzer` ⨉ 1-3 | draft_patches ≥1; **🔴写日志: phase=Draft** |
| **Precheck** | go build + go vet | ❌ | ❌ | — | build+vet 通过; **🔴写日志: phase=Precheck build=...** |
| **Act** | apply_patch 应用补丁 | ❌ | ❌ | — | 全部补丁已应用; **🔴写日志: phase=Act file=...** |
| **Evaluate** | go test 验证 + 输出分析 | ❌* | ❌* | `test-runner` ⨉ 1 | **🔴写日志: phase=Evaluate; 🔴输出 log_analysis + 可视化（级别由 output_detail 控制）** |

\* Evaluate 可读构建/测试输出；代码结构图谱仅在迭代 ≥2 时允许。
日志格式与可视化规范见 [logging.md](references/logging.md) / [log-analysis.md](references/log-analysis.md)。
...
│   │
│   └── [spawn_agent 不可用?] → 记录 spawn_failed 日志 → 主 Agent 本地执行
│       └── 两条日志：agent=<name> event=spawn_failed → agent=fallback event=fallback_local
│
...
│   ├── 单模块/多文件 → 1-2 个 module-code-analyzer
│   ├── 跨模块 → 2-3 个（范围不重叠时可并行）
│   ├── diagnose 类型 → 0 个 module-code-analyzer（跳过 Draft/Precheck/Act）
│   │   └── 直接从 Decide → Evaluate（输出诊断报告作为 log_analysis）
│   └── 代码审查 → 1 个 module-code-analyzer（只读）
│   └── 记录日志: phase=Decide task_type=... strategy=...
│
├── Precheck: go build ./... && go vet ./...（主 Agent 本地）
│   └── 若失败 → 带错误信息回 Draft
│   └── 记录日志: phase=Precheck build=...
│
├── Act: 每个文件 apply_patch（主 Agent 本地）
│   └── 记录日志: phase=Act file=... status=...
│
└── Evaluate: go test ./...（启动 test-runner）
    ├── 全部通过 → **输出 log_analysis + 6 图表** → 完成
    │   └── diagnose 类型：图表替换为诊断报告（问题清单 + 严重程度 + 建议）
    │       仍输出完整的 log_analysis JSON
    │   └── 记录日志: phase=Evaluate completeness=done
...
子 Agent 可用时严格遵循调度决策树。**子 Agent 不可用时**（`spawn_agent` 返回 unsupported 或模型 404）：
...
回退时必须写入**两条**日志（先 spawn_failed，再 fallback_local）：
...
**强制规则：** 即使 spawn_agent 工具不可用（工具列表中无 `multi_agent_v1_spawn_agent`），也必须跳过 spawn 尝试，直接写入 `spawn_failed` + `fallback_local` 两条日志后再开始本地执行。同时输出一条控制台消息说明回退原因。
...
| [logging.md](references/logging.md) | 10 类日志格式（kv 格式，含 spawn_failed/fallback_local） | 阶段转换时 |
