---
name: gen-index
description: 从 .codemap 文件、CodeMap 事实、目标源码读取和人工维护文档生成或更新 agent 面向项目索引。适用于创建 PROJECT_INDEX.md、NAVIGATION.md、CHANGE_GUIDE.md、FEATURES.md、AI 可读仓库索引、可选根目录 INDEX.md，或带澄清结论的项目索引文档。
---
...
4. 现有生成索引文件，例如 `.agent/PROJECT_INDEX.md`、`.agent/NAVIGATION.md`、`.agent/CHANGE_GUIDE.md`、`.agent/FEATURES.md` 或 `INDEX.md`，只作为增量更新的历史草稿。不要把它们当作事实层。
...
6. 把索引文件写成导航 artifact，而不是完整代码讲解。
7. 使用稳定的小节名称和紧凑的 bullet/table，让其他 skill 不需要猜测格式也能消费这些文件。
8. 验证列出的路径存在，或明确标记为 `unknown`、`generated`、`external`、`planned`。结束前对仓库路径使用 `rg --files` 或 `test -e <path>` 验证。
9. 报告变更文件、假设，以及 CodeMap 重新生成后需要刷新的部分。
...
- Freshness：仅在可从文件元数据、CodeMap 输出或用户上下文确认时记录生成日期；记录使用过的 CodeMap 来源路径或工具，以及何时视为过期。如果日期未知，写 `unknown`。
- Evidence：使用过的 CodeMap 文件/事实、文档和目标源码文件。
- Unknowns：需要确认或重新生成 CodeMap 的事实。

## 配套文件结构

事实可用时，用以下稳定字段编写配套文件。字段缺失时使用 `unknown` 或 `None found in CodeMap`，不要直接省略。
...
保留有用的用户补充说明，但在重写前用 `.codemap/` 和源码文件验证结构与路径。
```
